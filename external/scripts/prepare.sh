...
do
  case "$CompilationFile" in
    *".ц")
      ...
      Command="$TSIL $BuildDir/source/$LLCompilationFile --формат=ллвмір --архітектура=ікс86_64 --постачальник=загальний --система=лінукс скомпілювати $CompilationFile --біб=./БЦ"
      echo "$Command"
      $Command
