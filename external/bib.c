...
typedef void (*MEpollListenerBeforeDelete)(MEpoll* mepoll,
                                           MEpollListener* listener);
...
typedef struct MEpollListener {
  ...
  MEpollListenerBeforeDelete before_delete;
} MEpollListener;
...
void mepoll_delete_listener(MEpoll* mepoll, MEpollListener* listener) {
  if (listener->before_delete != NULL) {
    listener->before_delete(mepoll, listener);
  }
...
typedef uint8_t (*BeforeEachEventHandler)(MEpoll* mepoll, void* arg);

void mepoll_run(MEpoll* mepoll,
                BeforeEachEventHandler before_each,
                void* before_each_arg) {
  struct epoll_event events[MAX_EVENTS];

  while (true) {
    if (!before_each(mepoll, before_each_arg)) {
      return;
    }
...
typedef void (*ВідкликНаЗупинкуОбслуговувача)(Рушій* рушій,
                                              Слухач* обслуговувач,
                                              невідома_адреса аргумент);
typedef void (*ВідкликПередВидаленнямОбслуговувача)(Рушій* рушій,
                                                    Слухач* обслуговувач,
                                                    невідома_адреса аргумент);
...
typedef struct ДаніОбслуговувача {
  ...
  ВідкликПередВидаленнямОбслуговувача відклик_перед_видаленням;
...
void before_delete_tcp_server_listener_event_handler(MEpoll* mepoll,
                                                     MEpollListener* listener) {
  ДаніОбслуговувача* дані = (ДаніОбслуговувача*)listener->arg;
  дані->відклик_перед_видаленням(mepoll, listener, дані->аргумент);
  free(дані);
}
...
extern int мавка_біб_запустити_обслуговувач(
  ...
  дані->відклик_перед_видаленням = відклик_перед_видаленням;
  ...
  listener->before_delete = before_delete_tcp_server_listener_event_handler;
...
typedef void (*ВідкликНаДаніПідключення)(Рушій* рушій,
                                         ...
                                         size_t розмір,
                                         памʼять_п8 дані);
typedef void (*ВідкликПередВидаленнямПідключення)(Рушій* рушій,
                                                  Слухач* підключення,
                                                  невідома_адреса аргумент);
...
typedef struct ДаніПідключення {
  ...
  ВідкликПередВидаленнямПідключення відклик_перед_видаленням;
  логічне підключено;
} ДаніПідключення;
...
void before_delete_tcp_client_listener_event_handler(MEpoll* mepoll,
                                                     MEpollListener* listener) {
  ДаніПідключення* дані = (ДаніПідключення*)listener->arg;
  дані->відклик_перед_видаленням(mepoll, listener, дані->аргумент);
  free(дані);
}
...
extern int мавка_біб_підключити_підключення(
  ...
  MEpollListener* listener = mepoll_create_listener(
      рушій, sockfd, tcp_client_listener_event_handler, дані);
  listener->before_delete = before_delete_tcp_client_listener_event_handler;
...
extern int мавка_біб_відключити_підключення(Рушій* рушій, ц32 оф) {
  ...
  if (listener != NULL) {
    ДаніПідключення* дані = (ДаніПідключення*)listener->arg;
    дані->відклик_на_відключення(рушій, listener, дані->аргумент);
    mepoll_delete_listener(рушій, listener);
  }
...
typedef логічне (*ОбробникПодіїРушія)(Рушій* рушій,
...
void* мавка_біб_реєстр_інтернет_обслуговувачів = NULL;
void* мавка_біб_реєстр_інтернет_звʼязків = NULL;
void* мавка_біб_реєстр_планувальників = NULL;

extern void* мавка_біб_отримати_реєстр_інтернет_обслуговувачів() {
  return мавка_біб_реєстр_інтернет_обслуговувачів;
}

extern void мавка_біб_змінити_реєстр_інтернет_обслуговувачів(void* значення) {
  мавка_біб_реєстр_інтернет_обслуговувачів = значення;
}

extern void* мавка_біб_отримати_реєстр_інтернет_звʼязків() {
  return мавка_біб_реєстр_інтернет_звʼязків;
}

extern void мавка_біб_змінити_реєстр_інтернет_звʼязків(void* значення) {
  мавка_біб_реєстр_інтернет_звʼязків = значення;
}

extern void* мавка_біб_отримати_реєстр_планувальників() {
  return мавка_біб_реєстр_планувальників;
}

extern void мавка_біб_змінити_реєстр_планувальників(void* значення) {
  мавка_біб_реєстр_планувальників = значення;
}