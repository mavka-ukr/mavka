...
typedef struct ТурбоВихідІнтернетПідключення ТурбоВихідІнтернетПідключення;
...
typedef struct ТурбоВихідІнтернетПідключення {
  ц64 ідентифікатор;
  ю8 іа;
  п64 порт;
} ТурбоВихідІнтернетПідключення;
...
логічне турбо_створити_інтернет_підключення(
    ю8 іа,
    п16 порт,
    ТурбоВихідІнтернетПідключення* вихід_інтернет_підключення,
    ТурбоПомилка* вихід_помилки) {
  int sockfd = socket(AF_INET, SOCK_STREAM, 0);
  if (sockfd == -1) {
    вихід_помилки->код = errno;
  ...
  вихід_інтернет_підключення->ідентифікатор = sockfd;
  вихід_інтернет_підключення->іа = іа;
  вихід_інтернет_підключення->порт = порт;

  return true;
}

логічне
турбо_закрити_інтернет_підключення(ц32 ідентифікатор_інтернет_підключення,
                                   ТурбоПомилка* вихід_помилки) {
  int close_result = close(ідентифікатор_інтернет_підключення);
  if (close_result == -1) {
    вихід_помилки->код = errno;
    вихід_помилки->дані = NULL;
    return false;
...
логічне
турбо_перевірити_інтернет_підключення(ц32 ідентифікатор_інтернет_підключення,
                                      ТурбоПомилка* вихід_помилки) {
  int err;
  socklen_t len = sizeof(err);
  int result = getsockopt(ідентифікатор_інтернет_підключення, SOL_SOCKET,
                          SO_ERROR, &err, &len);
  if (result < 0 || err != 0) {
