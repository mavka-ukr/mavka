...
void турбо_надрукувати_ю8(ю8 значення) {
  позитивне п = 0;
...
typedef struct ТурбоВихідПеріодичного ТурбоВихідПеріодичного;
...
typedef void (*ТурбоВідкликПісляПодії)(ТурбоРушій* рушій,
                                       невідома_адреса аргумент_відклику);
...
typedef struct ТурбоВихідІнтернетЗвʼязку {
  ц64 ідентифікатор;
  ю8 іа;
  п64 порт;
...
typedef struct ТурбоВихідПеріодичного {
  ц64 ідентифікатор;
} ТурбоВихідПеріодичного;

ТурбоРушій* глобальний_рушій = NULL;

ТурбоРушій* турбо_глобальний_рушій() {
...
логічне турбо_видалити_слухач(ТурбоРушій* рушій,
  ...
  ТурбоВідкликПісляВидалення відклик_після_видалення =
      слухач->відклик_після_видалення;

  free(слухач);

  рушій->кількість_слухачів--;

  відклик_після_видалення(рушій, ідентифікатор, аргумент_відклику);
...
логічне турбо_обробити_чергу_запису(ТурбоРушій* рушій, ТурбоСлухач* слухач) {
    ...
    if (written == -1) {
      ...
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        int epoll_mod_result =
            epoll_ctl_mod(рушій->ідентифікатор, слухач->ідентифікатор,
                          EPOLLIN | EPOLLOUT | EPOLLET);
        if (epoll_mod_result == -1) {
        ...
        логічне результат_додавання_в_чергу = турбо_додати_запис_в_чергу(
            рушій, слухач, (памʼять_п8)дані, розмір, розмір_записаного,
...
логічне турбо_створити_інтернет_обслуговувач(
    ...
    ТурбоПомилка* вихід_помилки) {
  char* addr_str = перетворити_ю8_в_chars(іа);
...
логічне
турбо_закрити_інтернет_обслуговувач(ц32 ідентифікатор_інтернет_обслуговувача,
                                    ТурбоПомилка* вихід_помилки) {
...
логічне турбо_створити_інтернет_звʼязок(
    ю8 іа,
    п16 порт,
    ТурбоВихідІнтернетЗвʼязку* вихід_інтернет_звʼязку,
    ТурбоПомилка* вихід_помилки) {
  int sockfd = socket(AF_INET, SOCK_STREAM, 0);
  if (sockfd == -1) {
    вихід_помилки->код = errno;
  ...
  вихід_інтернет_звʼязку->ідентифікатор = sockfd;
  вихід_інтернет_звʼязку->іа = іа;
  вихід_інтернет_звʼязку->порт = порт;

  return true;
}

логічне
турбо_закрити_інтернет_звʼязок(ц32 ідентифікатор_інтернет_звʼязок,
                               ТурбоПомилка* вихід_помилки) {
  int close_result = close(ідентифікатор_інтернет_звʼязок);
  if (close_result == -1) {
    вихід_помилки->код = errno;
    вихід_помилки->дані = NULL;
    return false;
...
логічне
турбо_перевірити_інтернет_звʼязок(ц32 ідентифікатор_інтернет_звʼязку,
                                  ТурбоПомилка* вихід_помилки) {
  int err;
  socklen_t len = sizeof(err);
  int result = getsockopt(ідентифікатор_інтернет_звʼязку, SOL_SOCKET, SO_ERROR,
                          &err, &len);
  if (result < 0 || err != 0) {
    вихід_помилки->код = err;
    вихід_помилки->дані = NULL;
    return false;
  }
  return true;
...
логічне турбо_створити_періодичне(позитивне тривалість_очікування,
                                  ТурбоВихідПеріодичного* вихід_періодичного,
                                  ТурбоПомилка* вихід_помилки) {
  int tfd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK);
  if (tfd == -1) {
    perror("wtf");
  ...
  ts.it_value.tv_sec = (long int)тривалість_очікування / 1000;
  ts.it_value.tv_nsec = ((long int)тривалість_очікування % 1000) * 1000000;
  ts.it_interval.tv_sec = (long int)тривалість_очікування / 1000;
  ts.it_interval.tv_nsec = ((long int)тривалість_очікування % 1000) * 1000000;
  ...
  вихід_періодичного->ідентифікатор = tfd;

  return true;
}

логічне
турбо_закрити_періодичне(ц32 ідентифікатор_періодичного,
                         ТурбоПомилка* вихід_помилки) {
  int close_result = close(ідентифікатор_періодичного);
  if (close_result == -1) {
    вихід_помилки->код = errno;
    вихід_помилки->дані = NULL;
    return false;
...
логічне турбо_запустити_рушій(ТурбоРушій* рушій,
                              ТурбоВідкликПісляПодії відклик_після_події,
                              невідома_адреса аргумент_відклику,
                              ТурбоПомилка* вихід_помилки) {
    ...
    for (int i = 0; i < epoll_wait_result; i++) {
      ...
      if (слухач != NULL) {
        ...
        if (event & EPOLLOUT) {
          ...
          if (турбо_обробити_чергу_запису(рушій, слухач)) {
      ...
      if (відклик_після_події) {
        відклик_після_події(рушій, аргумент_відклику);
      }
    }
  }
...
void турбо_знищити_рушій(ТурбоРушій* рушій) {
  close(рушій->ідентифікатор);
  free(рушій);
}