...
ДУЖЕ ВАЖЛИВО: так як Ціль ще не має досконалої підтримки зовнішнії дій для взаємодії з _C (eng)_, усі значення які передаються через аргументи
...
В загальному: усі значення, окрім адреси, треба передавати як `природне`, `ціле` або `д64`. Але, якщо передається буфер, типу `памʼять<п8>`, тоді це нормально передавати його як `п8*`.
