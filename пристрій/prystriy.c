...
extern логічне пристрій_мавки_отримати_абсолютний_шлях(природне розмір_шляху,
  ...
  char абсолютний_шлях[PATH_MAX];
  if (realpath(шлях_рядок, абсолютний_шлях) == NULL) {
    free(шлях_рядок);
    return false;
  }
...
extern логічне пристрій_мавки_отримати_відносний_шлях(природне розмір_шляху,
  ...
  char поточна_тека[PATH_MAX];
  if (getcwd(поточна_тека, sizeof(поточна_тека)) == NULL) {
    free(шлях_рядок);
    return false;
  }
  ...
  if (realpath(шлях_рядок, абсолютний_шлях) == NULL) {
    ...
    strcpy(абсолютний_шлях, шлях_рядок);
  }
  free(шлях_рядок);
