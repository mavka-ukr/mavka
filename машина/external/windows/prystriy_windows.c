...
логічне перетворити_д64_в_ю8(д64 значення,
                             природне* вихід_розміру,
                             п8** вихід_даних) {
  // wsprintfA doesn't support floating point, so we need a different approach
  // Convert to string using a simple algorithm
  char буфер[64];

  // Handle negative numbers
  логічне відʼємне = (значення < 0);
  if (відʼємне) {
    значення = -значення;
  }

  // Get integer part
  ц64 ціла_частина = (ц64)значення;
  д64 дробова_частина = значення - (д64)ціла_частина;

  // Convert integer part
  char буфер_цілої[32];
  int позиція_цілої = 0;
  if (ціла_частина == 0) {
    буфер_цілої[позиція_цілої++] = '0';
  } else {
    ц64 тимчасова = ціла_частина;
    int лічильник = 0;
    while (тимчасова > 0) {
      буфер_цілої[лічильник++] = '0' + (тимчасова % 10);
      тимчасова /= 10;
    }
    // Reverse
    for (int і = 0; і < лічильник; і++) {
      буфер[позиція_цілої++] = буфер_цілої[лічильник - 1 - і];
    }
  }
  ...
  for (int і = 0; і < 6; і++) {
    дробова_частина *= 10;
    int цифра = (int)дробова_частина;
    буфер[позиція_цілої++] = '0' + цифра;
    дробова_частина -= цифра;
  }

  // Build final string
  int довжина = 0;
  char фінальний_буфер[64];
  if (відʼємне) {
    фінальний_буфер[довжина++] = '-';
  }
  for (int і = 0; і < позиція_цілої; і++) {
    фінальний_буфер[довжина++] = буфер[і];
  }
  ...
  *вихід_даних = (п8*)HeapAlloc(GetProcessHeap(), 0, *вихід_розміру);
  ...
  for (природне і = 0; і < *вихід_розміру; і++) {
    (*вихід_даних)[і] = (п8)фінальний_буфер[і];
  }
  return TRUE;
}
